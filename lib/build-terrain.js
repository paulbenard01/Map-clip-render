...
async function main() {
  ...
  if (!fs.existsSync(tifPath) || !fs.existsSync(tfwPath)) {
    ...
    try {
      ...
      // Plain Windows cmd/PowerShell (no WSL, no git-bash) has no `unzip`
      // — but every Windows install since 8.1 ships PowerShell, which has
      // its own extractor. Try that before giving up. It pulls in a couple
      // of extra files from the zip (a readme, a license) alongside the
      // two this script actually needs; harmless, left alongside them.
      try {
        const psPath = (p) => "'" + p.replace(/'/g, "''") + "'";
        execFileSync("powershell", [
          "-NoProfile", "-Command",
          `Expand-Archive -LiteralPath ${psPath(zipPath)} -DestinationPath ${psPath(TERRAIN_DIR)} -Force`,
        ], { stdio: "inherit" });
      } catch (err2) {
        console.error(
          `\nCouldn't run 'unzip' or PowerShell's Expand-Archive. Extract ${ZIP_NAME}\n` +
          `yourself (any zip tool) into ${path.relative(ROOT, TERRAIN_DIR)}/ and re-run this script.`
        );
        process.exit(1);
      }
    }
  ...
  // clamped back to the valid range. The renderer samples this raster
  // per-pixel against its own inverse projection rather than handing it to
  // a GPU as a single texture, so there's no tiling/texture-size limit and
  // no Mercator-style polar singularity to work around: full pole-to-pole
  // coverage renders correctly under Equal Earth.
  const west = clamp(wf.originX, -180, 180);
  const north = clamp(wf.originY, -90, 90);
  const east = clamp(wf.originX + wf.pixelWidth * meta.width, -180, 180);
  const south = clamp(wf.originY + wf.pixelHeight * meta.height, -90, 90);

  // Full native resolution — the renderer reprojects this per-pixel on its
  // own CPU-side grid (see lib/equal-earth-map.js) rather than handing it
  // to a GPU as a single texture, so there's no tiling/texture-size limit
  // forcing a downscale here the way there would be under WebGL.
  ...
  fs.writeFileSync(outBounds, JSON.stringify({
    // Corner order lib/equal-earth-map.js expects: top-left, top-right,
    // bottom-right, bottom-left — used to map the equirectangular pixel
    // grid to real lng/lat for its own per-pixel reprojection.
