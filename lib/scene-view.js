...
(function (root, factory) {
  ...
  /**
   * Adds or removes the terrain raster after the map has already loaded —
   * used by the builder when the user toggles the "terrain" scene flag
   * on/off, rather than only at initial setup like installBasemap.
   *
   * `terrain` is `{ url, coordinates }` to show it, or a falsy value to
   * remove it and restore full land/country fill opacity.
   */
  function setTerrain(map, terrain) {
    if (terrain) {
      if (!map.getSource("terrain")) {
        map.addSource("terrain", { type: "image", url: terrain.url, coordinates: terrain.coordinates });
        // Inserted below the land fill so it stays under borders/highlights.
        map.addLayer({ id: "terrain-raster", type: "raster", source: "terrain", paint: { "raster-opacity": 1 } }, "land-fill");
      }
    } else if (map.getSource("terrain")) {
      if (map.getLayer("terrain-raster")) map.removeLayer("terrain-raster");
      map.removeSource("terrain");
    }
    var hasTerrain = !!map.getSource("terrain");
    if (map.getLayer("land-fill")) map.setPaintProperty("land-fill", "fill-opacity", hasTerrain ? TERRAIN_LAND_OPACITY : 1);
    if (map.getLayer("countries-fill")) map.setPaintProperty("countries-fill", "fill-opacity", hasTerrain ? TERRAIN_LAND_OPACITY : 1);
  }
