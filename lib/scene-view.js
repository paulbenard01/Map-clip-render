/**
 * scene-view.js — turns a scene into pixels on the map.
 *
 ...
 *
 * Deliberately DOM/SVG rather than a map library's own symbol layers: a
 * remote font-glyph server is the usual reason online map tools need an
 * internet connection even for "static" text, which would mean this tool
 * couldn't render offline. Building the text ourselves sidesteps that and
 * gives full control over the look. See README.
 *
 * The `map` this module paints onto is lib/equal-earth-map.js's
 * EqualEarthMap — this file only ever calls a small, generic slippy-map-
 * shaped API (addSource/addLayer/project/jumpTo/...) and never references
 * that implementation directly, which is what let the underlying renderer
 * be swapped from MapLibre/Mercator to Equal Earth without changing
 * anything here.
 */
(function (root, factory) {
  ...
  // At full terrain visibility, land/country fills and highlight fills fade
  // to these floors rather than 0 — enough to still read as "this is India"
  // or "this is the highlighted country" while the relief shows through.
  // Highlights get a higher floor than plain land so the highlight colour
  // stays legible as a highlight, not just a faint tint.
  var MIN_LAND_OPACITY = 0.35;
  var MIN_HIGHLIGHT_OPACITY = 0.55;
   ...
   *
   * `data` is { land, countries, graticule, terrain, terrainOpacity } as
   * already-parsed GeoJSON (plus terrain's own shape below) — the caller
   * fetches them, because the builder keeps them cached across style
   ...
   *
   * `data.terrainOpacity` (0..1, default 1) is the scene's global terrain
   * slider — 0 is a flat map even when the asset is present, 1 is full
   * relief. Only meaningful alongside `data.terrain`.
   */
  function installBasemap(map, preset, data) {
    var terrainOpacity = data.terrainOpacity == null ? 1 : data.terrainOpacity;
    if (data.terrain && !map.getSource("terrain")) {
      ...
      map.addLayer({ id: "terrain-raster", type: "raster", source: "terrain", paint: { "raster-opacity": terrainOpacity } });
    }
    ...
    var landOpacity = data.terrain ? lerp(1, MIN_LAND_OPACITY, terrainOpacity) : 1;
    if (!map.getSource("land")) {
      map.addSource("land", { type: "geojson", data: data.land });
      map.addLayer({ id: "land-fill", type: "fill", source: "land", paint: { "fill-color": preset.land, "fill-opacity": landOpacity } });
    }
    if (!map.getSource("countries")) {
      map.addSource("countries", { type: "geojson", data: data.countries });
      map.addLayer({ id: "countries-fill", type: "fill", source: "countries", paint: { "fill-color": preset.land, "fill-opacity": landOpacity } });
      // A dark halo just under the actual border line — invisible on the
      // flat map (opacity 0 there), fading in with the terrain slider. The
      // border colour alone is a thin, fairly light line, easy to lose
      // against hillshade texture busy enough to have similarly light
      // patches of its own; the halo gives it contrast against any
      // background rather than just the flat fill colour it was tuned for.
      map.addLayer({
        id: "countries-border-halo", type: "line", source: "countries",
        paint: { "line-color": "rgba(6,10,16,0.6)", "line-width": (preset.borderWidth || 0.6) + 2.4, "line-opacity": data.terrain ? terrainOpacity : 0 },
      });
  ...
  /**
   * Live-updates the terrain raster's opacity and the land/country/
   * highlight fill opacities that blend with it — called whenever the
   * scene's terrainOpacity slider changes, not just at initial setup.
   * A no-op (opacities pinned to fully opaque) when there's no terrain
   * source at all.
   */
  function setTerrainOpacity(map, terrainOpacity) {
    var hasTerrain = !!map.getSource("terrain");
    var op = hasTerrain ? (terrainOpacity == null ? 1 : terrainOpacity) : 0;
    if (map.getLayer("terrain-raster")) map.setPaintProperty("terrain-raster", "raster-opacity", op);
    var landOpacity = hasTerrain ? lerp(1, MIN_LAND_OPACITY, op) : 1;
    if (map.getLayer("land-fill")) map.setPaintProperty("land-fill", "fill-opacity", landOpacity);
    if (map.getLayer("countries-fill")) map.setPaintProperty("countries-fill", "fill-opacity", landOpacity);
    if (map.getLayer("countries-border-halo")) map.setPaintProperty("countries-border-halo", "line-opacity", op);
  }
  ...
  function applyPresetToBasemap(map, preset) {
    ...
    if (map.getLayer("countries-border-halo")) {
      map.setPaintProperty("countries-border-halo", "line-width", (preset.borderWidth || 0.6) + 2.4);
    }
  }
   ...
   *
   *   map        a loaded EqualEarthMap (lib/equal-earth-map.js)
  ...
  function createStage(opts) {
    ...
    var hatchPatterns = {};   // colour -> registered map image id
    ...
    /**
     * A small diagonal-hatch tile in the given colour, registered with the
     * map as a fill pattern. Cached per colour so switching scenes
    ...
    function rebuildZones() {
      ...
      scene.zones.forEach(function (z) {
        ...
        }, map.getLayer("countries-border-halo") ? "countries-border-halo" : undefined);
        map.addLayer({
    ...
    function setScene(nextScene) {
      ...
      clearOverlays();
      // A new scene's ids and geometry have nothing to do with the old
      // one's — carrying over easing state would just make the first
      // frame's nudges (wrongly) ease in from wherever the last scene
      // left off, or briefly reference a since-removed element's offset.
      appliedOffsets = {};
      lastResolveT = null;
      ...
      rebuildHighlights();
      rebuildZones();
      setTerrainOpacity(map, scene.terrainOpacity);
    }
    ...
    function setFrame(t, opts2) {
      ...
      // Country highlights — capped so a fully-faded-in highlight still
      // lets terrain show through underneath it, the same way plain land
      // does, rather than painting over it solidly.
      var highlightCap = map.getSource("terrain") ? lerp(1, MIN_HIGHLIGHT_OPACITY, scene.terrainOpacity) : 1;
      Engine.countryHighlightsAt(scene, t).forEach(function (h) {
        var layerId = "highlight-" + h.id;
        if (map.getLayer(layerId)) map.setPaintProperty(layerId, "fill-opacity", h.opacity * highlightCap);
      });
      ...
      Engine.zonesAt(scene, t).forEach(function (z) {
        ...
        if (entry && entry.labelEl) {
          ...
          entry.labelEl.style.opacity = z.opacity > 0 && isElementOnScreen(entry.labelEl) ? z.opacity : 0;
        }
      });
      ...
      Object.keys(routeEls).forEach(function (id) {
        ...
        if (entry.labelEl) {
          ...
          // so it doesn't end up sitting on the destination pin. That
          // midpoint is a fixed geographic point, unrelated to the current
          // camera framing — a camera move made for some other reason (a
          // pull-back to fit a pin near an endpoint, say) can leave it
          // sitting well outside the frame with nothing connecting it to
          // anything on screen. Better to fade it out than leave a caption
          // floating in empty space for the rest of the clip.
          ...
          entry.labelEl.style.opacity = isElementOnScreen(entry.labelEl) ? st.labelOpacity : 0;
        }
      });

      resolveLabelOverlaps(t);
    }

    // Per-candidate id -> the dy currently applied, carried across frames so
    // a change in target can be eased in rather than snapped to instantly.
    // Persists for the life of the stage; a stale entry for an id that's no
    // longer visible is harmless (just unused until/unless it reappears).
    var appliedOffsets = {};
    var lastResolveT = null;
    var NUDGE_MARGIN = 6;       // keep a nudged box at least this far inside the frame edge
    var MAX_NUDGE_SPEED = 260;  // px/second — how fast a label eases to a new offset

    /** Whether `el`'s current position puts any part of it within (or near)
     * the visible frame — used to hide a route/zone label whose anchor is a
     * fixed geographic point that a camera move can leave stranded well
     * outside the frame, with nothing else on screen for it to relate to. */
    function isElementOnScreen(el) {
      var margin = 20;
      var fr = overlay.getBoundingClientRect();
      var r = el.getBoundingClientRect();
      if (r.width === 0 && r.height === 0) return true; // not laid out yet; nothing to hide
      // The box's centre, not just any sliver of it, has to be within (or
      // near) the frame -- a box that's mostly off-frame but still grazing
      // the edge by a few pixels shouldn't count as "visible".
      var cx = (r.left + r.right) / 2, cy = (r.top + r.bottom) / 2;
      return cx > fr.left - margin && cx < fr.right + margin && cy > fr.top - margin && cy < fr.bottom + margin;
    }

    /**
     * A badge pin's card is anchored a fixed distance to the *right* of its
     * marker in CSS — fine until the marker itself lands in the right
     * portion of the frame (a pin the camera is centred on, say), at which
     * point the card runs straight off the edge. resolveLabelOverlaps()
     * only ever nudges things vertically, so it can't fix this; this flips
     * the card to the marker's left side instead whenever the right-anchored
     * version wouldn't fit.
     */
    function flipCardsNearEdge() {
      var frameRect = overlay.getBoundingClientRect();
      Object.keys(pinEls).forEach(function (id) {
        var entry = pinEls[id];
        var card = entry.el.querySelector(".pin-card");
        if (!card || parseFloat(entry.el.style.opacity || "0") <= 0.05) return;
        card.classList.remove("pin-card-flip");
        var r = card.getBoundingClientRect();
        if (r.width === 0) return;
        if (r.right > frameRect.right - NUDGE_MARGIN) card.classList.add("pin-card-flip");
      });
    }
     ...
     * capture.
     *
     * Two things that used to make this look broken in an actual clip,
     * rather than just a single static screenshot:
     *
     * - The set of obstacles changes every frame as things fade in and
     *   out, so the *target* offset for a label can jump abruptly between
     *   one frame and the next (e.g. a new pin fades in right where a
     *   label used to sit uncontested). Snapping straight to the new
     *   target reads as a glitch. Now the applied offset eases toward the
     *   target at a bounded speed (MAX_NUDGE_SPEED), so a change in target
     *   becomes a smooth, fast slide instead of a jump — still fully
     *   deterministic frame-to-frame since it only depends on t and the
     *   time delta since the last call.
     * - The old search only ever pushed a label *down*, with no limit —
     *   with enough stacked obstacles (several close pins, a lot of
     *   simultaneous routes) that runs a label straight off the bottom of
     *   the frame. findClearOffset() below now searches down and up, and
     *   stays within the visible frame.
     *
     ...
     */
    function resolveLabelOverlaps(t) {
      var dt = (t == null || lastResolveT == null || t < lastResolveT) ? null : (t - lastResolveT);
      lastResolveT = t;
      var maxStep = dt == null ? Infinity : Math.max(3, dt * MAX_NUDGE_SPEED);

      var frameRect = overlay.getBoundingClientRect();
      var placed = [];
      // A pin's own marker (photo circle, badge dot) never moves — it has
      // to stay exactly on its coordinate — but it's a real obstacle other
      // elements need to avoid. Without this, a badge card from a nearby
      // pin can sit directly on top of another pin's photo with nothing
      // to push it clear, which is exactly the case two genuinely close
      // real-world sites (a city and a site just outside it) hit.
      scene.pins.forEach(function (pin) {
        var entry = pinEls[pin.id];
        if (!entry || parseFloat(entry.el.style.opacity || "0") <= 0.05) return;
        var marker = entry.el.querySelector(".pin-photo, .pin-dot");
        if (!marker) return;
        var r = marker.getBoundingClientRect();
        if (r.width === 0 && r.height === 0) return;
        placed.push({ top: r.top, bottom: r.bottom, left: r.left, right: r.right });
      });
      labelCandidates().forEach(function (c) {
        ...
        var target = findClearOffset(box, placed, frameRect);
        var prev = appliedOffsets[c.id] || 0;
        var applied;
        if (dt == null || target === prev) {
          applied = target;
        } else {
          var diff = target - prev;
          applied = prev + Math.sign(diff) * Math.min(Math.abs(diff), maxStep);
        }
        appliedOffsets[c.id] = applied;

        if (applied !== 0) c.el.style.transform = (c.base ? c.base + " " : "") + "translateY(" + applied + "px)";
        placed.push(shiftBox(box, applied));
      });
    }

    /**
     * Where to put `box` so it clears every box in `placed`, staying inside
     * `frameRect` — tries increasing offsets both down and up (down first,
     * since that reads more naturally for text under a pin) rather than
     * only ever pushing one direction. If nothing fully in-bounds is also
     * fully clear, prefers an in-bounds spot with some residual overlap
     * over letting the box leave the frame entirely.
     */
    function findClearOffset(box, placed, frameRect) {
      var step = (box.bottom - box.top) + 4;
      function inBounds(dy) {
        return box.top + dy >= frameRect.top + NUDGE_MARGIN && box.bottom + dy <= frameRect.bottom - NUDGE_MARGIN;
      }
      function overlapsAny(dy) {
        return placed.some(function (p) { return boxesOverlap(shiftBox(box, dy), p); });
      }
      if (inBounds(0) && !overlapsAny(0)) return 0;
      for (var i = 1; i <= 6; i++) {
        var down = i * step, up = -i * step;
        if (inBounds(down) && !overlapsAny(down)) return down;
        if (inBounds(up) && !overlapsAny(up)) return up;
      }
      for (var j = 6; j >= 1; j--) {
        if (inBounds(j * step)) return j * step;
        if (inBounds(-j * step)) return -j * step;
      }
      return 0; // boxed in on every side; sit at the natural spot rather than clamp arbitrarily
    }
    ...
    function labelCandidates() {
      ...
      scene.pins.forEach(function (pin) {
        ...
        if (node) out.push({ el: node, base: "", id: "pin:" + pin.id });
      });
      scene.routes.forEach(function (route) {
        var entry = routeEls[route.id];
        if (!entry || !entry.labelEl || !visible(entry.labelEl)) return;
        out.push({ el: entry.labelEl, base: "translate(-50%, -50%)", id: "route:" + route.id });
      });
      scene.zones.forEach(function (zone) {
        var entry = zoneEls[zone.id];
        if (!entry || !entry.labelEl || !visible(entry.labelEl)) return;
        out.push({ el: entry.labelEl, base: "translate(-50%, -50%)", id: "zone:" + zone.id });
      });
      scene.titles.forEach(function (title) {
        ...
        out.push({ el: entry.el, base: base, id: "title:" + title.id });
      });
      return out;
    }
