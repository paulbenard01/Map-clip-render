...
(function (root, factory) {
  ...
  EqualEarthMap.prototype._resizeCanvas = function () {
    ...
    this._width = Math.max(1, Math.round(rect.width));
    this._height = Math.max(1, Math.round(rect.height));
  ...
  EqualEarthMap.prototype._drawRaster = function (layer) {
    ...
    var workW = Math.min(RASTER_WORK_WIDTH, this._width);
    var workH = Math.max(1, Math.round(workW * this._height / this._width));
    if (!this._rasterWork) {
      this._rasterWork = document.createElement("canvas");
      this._rasterWorkCtx = this._rasterWork.getContext("2d");
    }
    this._rasterWork.width = workW;
    this._rasterWork.height = workH;
    var out = this._rasterWorkCtx.createImageData(workW, workH);

    var scaleX = this._width / workW, scaleY = this._height / workH;
    var cx = this._width / 2, cy = this._height / 2;
    var sw = src.width, sh = src.height;

    for (var oy = 0; oy < workH; oy++) {
      for (var ox = 0; ox < workW; ox++) {
        var fx = ox * scaleX, fy = oy * scaleY;
        var r = rotatePoint(fx, fy, cx, cy, -this._bearingRad);
        var ll = this._projection.invert(r);
        if (!ll) continue;
    ...
    this._rasterWorkCtx.putImageData(out, 0, 0);

    var ctx = this._ctx;
    ctx.save();
    ctx.globalAlpha = opacity;
    ctx.imageSmoothingEnabled = true;
    ctx.drawImage(this._rasterWork, 0, 0, workW, workH, 0, 0, this._width, this._height);
    ctx.restore();
  };
  ...
  EqualEarthMap.prototype._installInteraction = function (dragEnabled) {
    ...
    canvas.addEventListener("pointerdown", function (e) {
      ...
      anchor = self.unproject([e.clientX - rect.left, e.clientY - rect.top]);
    ...
    canvas.addEventListener("pointerup", function (e) {
      ...
      if (!moved) {
        var rect = canvas.getBoundingClientRect();
        self._fire("click", { lngLat: self.unproject([e.clientX - rect.left, e.clientY - rect.top]) });
      }
    });
