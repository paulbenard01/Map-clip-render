...
/** Builds the offline basemap if it isn't there yet (first-run convenience). */
function ensureBasemap(log) {
  if (fs.existsSync(path.join(ROOT, "data", "land.geo.json"))) return;
  log("Basemap data not found, building it now (one-time step)...");
...
async function renderScene(options) {
  ...
  const { width, height } = resolveDimensions(ratio, opts.width, draft);
