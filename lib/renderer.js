...
async function renderScene(options) {
  ...
  if (opts.scenePath) {
    ...
    // A scene's own `name` wins over its filename, so a clip filed under
    // scenes/sarnath/04-circuit.json still renders to sarnath-04-circuit.mp4
    // rather than an ambiguous 04-circuit.mp4 sitting in output/.
    sceneName = scene.name || path.basename(scenePath, ".json");
  } else if (opts.scene) {
    scene = opts.scene;
