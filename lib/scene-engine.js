...
(function (root, factory) {
  ...
  var DEFAULTS = {
    ...
    zoneRadius: 200,       // km
    zoneFade: 0.6,
    zoneOpacity: 0.35,     // the hatch fill's own alpha, on top of the fade
  ...
  var CAMERA_TRANSITIONS = ["ease", "cut", "zoomBlast", "smooth", "organic"];
  var TITLE_POSITIONS = ["bottom-left", "bottom-right", "top-left", "top-right", "center", "custom"];
  ...
  // A longer hold at both ends and a faster middle than easeInOutCubic —
  // reads as a slower, more deliberate glide rather than a brisk pan.
  function easeInOutQuint(x) {
    return x < 0.5 ? 16 * x * x * x * x * x : 1 - Math.pow(-2 * x + 2, 5) / 2;
  }
  // Overshoots past 1 then settles back — the camera arrives, drifts slightly
  // past the target, and eases home, rather than stopping dead on arrival.
  function easeOutBack(x) {
    var c1 = 1.70158, c3 = c1 + 1;
    return 1 + c3 * Math.pow(x - 1, 3) + c1 * Math.pow(x - 1, 2);
  }
  // Gentle S-curve — zero velocity at both ends, unlike a linear ramp, which
  // is what actually reads as "robotic" for something fading or drawing on.
  ...
  function normalizeScene(scene) {
    ...
    var out = {
      ...
      // How much the terrain raster shows through, 0 (flat land colour) to
      // 1 (full relief) — has no effect until `npm run build-terrain` has
      // been run locally. See lib/scene-view.js for how this blends with
      // land/country fill and highlight opacity.
      terrainOpacity: clamp01(num(s.terrainOpacity, 1)),
      camera: [],
      pins: [],
      routes: [],
      countryHighlights: [],
    ...
    out.pins = (s.pins || []).map(function (p, i) {
      return {
        ...
        labelCaps: p.labelCaps !== false,
        labelBold: p.labelBold !== false,
      };
    });
    ...
    out.routes = (s.routes || []).map(function (r, i) {
      return {
        ...
        arrowEnd: !!r.arrowEnd,
        label: r.label || null,
        labelCaps: r.labelCaps !== false,
        labelBold: r.labelBold !== false,
      };
    });

    out.titles = (s.titles || []).map(function (t, i) {
      var position = TITLE_POSITIONS.indexOf(t.position) >= 0 ? t.position : DEFAULTS.titlePosition;
      return {
        id: t.id || "title" + i,
        text: t.text || "",
        position: position,
        // Free-form placement: normalized (0..1) fractions of the frame,
        // only meaningful when position === "custom". Set by dragging the
        // title directly on the canvas, or by hand for precise values.
        x: position === "custom" ? num(t.x, 0.5) : null,
        y: position === "custom" ? num(t.y, 0.5) : null,
        at: num(t.at, 0),
        until: t.until == null ? null : num(t.until, null),
        fade: t.fade == null ? null : num(t.fade, null),
        size: num(t.size, DEFAULTS.titleSize),
        // Every overlay text element defaults to the series' current look
        // (all-caps, bold) so existing scenes render unchanged; either can
        // be turned off per title.
        caps: t.caps !== false,
        bold: t.bold !== false,
        color: t.color || null,
      };
    ...
    var highlights = (s.countryHighlights || []).map(function (h, i) {
      ...
      // Carried through so normalize stays idempotent — the builder
      // re-normalises after every edit, and losing this would quietly
      // rewrite a v1 scene's focusCountry into a countryHighlights array.
      if (h._fromFocusCountry) out._fromFocusCountry = true;
      return out;
    ...
    out.zones = (s.zones || []).map(function (z, i) {
      // A zone is either a geodesic circle (centre + radius) or an
      // arbitrary polygon (an explicit list of points) — a "sphere of
      // influence" is rarely actually round: a contested border region, a
      // cultural corridor, a coastline strip. Shape is inferred from
      // whether usable points were given, rather than trusted from a
      // `shape` field, so a scene can't claim "polygon" with no points.
      var points = (Array.isArray(z.points) ? z.points : []).map(coord).filter(Boolean);
      var isPolygon = points.length >= 3;
      return {
        id: z.id || "zone" + i,
        shape: isPolygon ? "polygon" : "circle",
        points: isPolygon ? points : [],
        center: isPolygon ? centroid(points) : (coord(z.center) || [0, 0]),
        radius: num(z.radius, DEFAULTS.zoneRadius),
        color: z.color || null,           // null => falls back to the preset's accent
        label: z.label || null,
        at: num(z.at, 0),
        until: z.until == null ? null : num(z.until, null),
        fade: num(z.fade, DEFAULTS.zoneFade),
      };
    });
  ...
  /** Simple average of a polygon's points — good enough for label placement. */
  ...
  function serializeScene(scene) {
    ...
    out.style = s.style;
    out.fps = s.fps;
    if (s.terrainOpacity !== 1) out.terrainOpacity = s.terrainOpacity;
    ...
    out.pins = s.pins.map(function (p, i) {
      ...
      if (!p.labelCaps) o.labelCaps = false;
      if (!p.labelBold) o.labelBold = false;
      return o;
    });
    ...
    out.routes = s.routes.map(function (r, i) {
      ...
      if (!r.labelCaps) o.labelCaps = false;
      if (!r.labelBold) o.labelBold = false;
      return o;
    });
    ...
    out.titles = s.titles.map(function (t, i) {
      ...
      if (!t.caps) o.caps = false;
      if (!t.bold) o.bold = false;
    ...
    out.zones = s.zones.map(function (z, i) {
      var o = {};
      if (z.id && z.id !== "zone" + i) o.id = z.id;
      if (z.shape === "polygon") {
        o.points = z.points.map(roundCoord);
      } else {
        o.center = roundCoord(z.center);
        if (z.radius !== DEFAULTS.zoneRadius) o.radius = round(z.radius, 1);
      }
      if (z.color) o.color = z.color;
      if (z.label) o.label = z.label;
      o.at = round(z.at, 3);
      o.until = z.until == null ? null : round(z.until, 3);
      if (z.fade !== DEFAULTS.zoneFade) o.fade = round(z.fade, 3);
      return o;
    });
   ...
   *   smooth     a slower, more deliberate glide (easeInOutQuint on every
   *              channel) — holds longer at both ends than "ease"
   *   organic    the camera drifts slightly past the target and eases back
   *              (easeOutBack on every channel) — a soft, human landing
   *              rather than stopping dead on arrival
   *
  ...
  function cameraAt(scene, t) {
    ...
    for (var i = 0; i < kfs.length - 1; i++) {
      ...
      if (t >= a.t && t <= b.t) {
        ...
        var f = transition === "smooth" ? easeInOutQuint(raw)
          : transition === "organic" ? easeOutBack(raw)
          : easeInOutCubic(raw);
   ...
   *
   * The ramp itself is eased (smoothstep — zero velocity at both ends)
   * rather than linear. A linear fade changes brightness at a constant
   * rate the whole way, which reads as mechanical; easing it is what a
   * real dissolve looks like. Same fadeSec, same 0-at-`at` and 1-once-
   * settled boundaries — only the shape of the ramp between them changes.
   */
  function fadeOpacity(t, at, until, fadeSec) {
    ...
    var inT = smoothstep(clamp01((t - at) / fadeSec));
    var outT = until === Infinity ? 1 : smoothstep(clamp01((until - t) / fadeSec));
  ...
  function routeStateAt(route, t) {
    ...
    // Eased rather than constant-speed: the line gathers pace through the
    // middle of its draw and settles as it completes, instead of unrolling
    // at a perfectly steady rate the whole way.
    var drawFrac = drawDur > 0 ? easeInOutCubic(clamp01((t - start) / drawDur)) : 1;
  ...
  // -------------------------------------------------------------- zones
  /**
   * A "sphere of influence": a region defined by a centre and a radius,
   * rather than a country border — the thing a country highlight can't
   * express (a zone of control that spans parts of several countries, or
   * is smaller than one). Returns a closed ring of [lng, lat] points
   * forming a geodesic circle — the direct-geodesic formula (bearing +
   * angular distance from centre), the same spherical approach
   * sampleGreatCircle uses for routes, not a flat ellipse in lng/lat space,
   * which would visibly distort away from the equator.
   */
  function zoneCircle(center, radiusKm, steps) {
    steps = steps || 64;
    var lat1 = center[1] * D2R, lon1 = center[0] * D2R;
    var delta = radiusKm / EARTH_RADIUS_KM;
    var ring = [];
    var prevLng = center[0];
    for (var i = 0; i <= steps; i++) {
      ...
      var lngDeg = unwrapLng(lon2 * R2D, prevLng);
      prevLng = lngDeg;
      ring.push([lngDeg, lat2 * R2D]);
    }
    return ring;
  }
  ...
  /** Active zones at time t, with their current opacity and geometry. */
  function zonesAt(scene, t) {
    return ((scene && scene.zones) || []).map(function (z) {
      return {
        id: z.id, color: z.color, label: z.label, center: z.center, radius: z.radius, shape: z.shape,
        opacity: fadeOpacity(t, z.at, z.until, z.fade),
        ring: zoneRing(z),
      };
    });
  }
  ...
  /**
   * A zone's boundary as a closed ring of [lng, lat] points — a geodesic
   * circle for "circle", or the zone's own points, closed and kept
   * continuous across the antimeridian, for "polygon".
   */
  function zoneRing(zone) {
    if (zone.shape === "polygon" && zone.points && zone.points.length >= 3) {
      var prevLng = zone.points[0][0];
      var ring = zone.points.map(function (p) {
        var lng = unwrapLng(p[0], prevLng);
        prevLng = lng;
        return [lng, p[1]];
      });
    ...
    return zoneCircle(zone.center, zone.radius);
  }
  ...
  function computeDuration(scene) {
    ...
    ((scene && scene.zones) || []).forEach(function (z) {
      if (typeof z.until === "number") ends.push(z.until + 0.5);
    });
  ...
  function timelineBlocks(scene) {
    ...
    (scene.zones || []).forEach(function (z, i) {
      var fallback = z.shape === "polygon" ? z.points.length + "-point region" : Math.round(z.radius) + "km";
      blocks.push({ track: "zones", kind: "zone", id: z.id, index: i, start: z.at, end: z.until == null ? duration : z.until, openEnded: z.until == null, label: z.label || fallback });
    });
